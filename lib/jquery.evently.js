...
(function($) {
  ...
  function renderElement(me, h, args, qrun, arun) {
    ...
    if (h.async && !arun) {
      ...
      var rendered;
      $.forIn($.evently.callbacks.render, function(name, fun) {
        if (h[name]) {
          rendered = fun.apply(me, [h, args]);
        }
      });
      
      $.forIn($.evently.callbacks.after, function(name, fun) {
        if (h[name]) {
  ...
  // default callbacks for the render phase
  $.evently.callbacks = {
    before : {},
    // before : {
    //   before : function() {},
...
(function($) {
  ...
  $.evently.callbacks.render.mustache = function(h, args) {
...
(function($) {
  $.evently.callbacks.after.selectors = function(h, rendered, args) {
    var render = (h.render || "replace").replace(/\s/g,"")
...
(function($) {
  $.evently.callbacks.after.after = function(h, rendered, args) {
