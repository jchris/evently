...
(function($) {
  $.evently.fn.before.async = function(h, cb, args) {
    $.evently.utils.funViaString(h.async, this).apply(this, [cb].concat($.argsToArray(args)));
  };
})(jQuery);
...
(function($) {  
  function runQuery(me, h, args) {
    ...
    var app = $$(me).app;
    var qu = $.evently.utils.runIfFun(me, h.query, args);
    var qType = qu.type;
  ...
  $.evently.fn.before.query = function(h, cb, args) {
    var app = $$(this).app
      , qu = $.evently.utils.runIfFun(this, h.query, args)
      , qType = qu.type
      , viewName = qu.view
